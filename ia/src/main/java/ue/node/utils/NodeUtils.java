...
public class NodeUtils {
	...
	public static Boolean exist(Node node) {
		...
		return runner.query(cypher, new IsExistHandler(),
				new Object[] { nodeName });
	}
	...
	public static void createRelation(Node node) {
		...
		// MATCH (a:Person) where a.name = ""  MERGE (a)-[r:KNOWS]->(b:Person {name:""})
		String cypher = "";
		...
		while (it.hasNext()) {
			...
			//String relIdentify = entry.getKey();  //write...write....Person.name=Article.author
			Relation rel = entry.getValue();
			cypher = "MATCH (a:"+ node.getName() +" {"+ rel.getHeadNodePro()+": ?}), (b:"+ rel.getTailNodeType() +" {"+ rel.getTailNodePro() +": ?}) CREATE (a)-[r:"+ rel.getName() +"]->(b)";
	...
	public static void updateNode(Node node, String condition) {
		...
		// MATCH (n:`Author` { name: 'liuchao' }) SET n.address = "Andres", n.province = "Developer";
		StringBuffer sb = new StringBuffer();
